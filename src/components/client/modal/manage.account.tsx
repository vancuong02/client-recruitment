...
const UserUpdateInfo = () => {
    ...
    return (
        <Form form={form} name="user-update-info" onFinish={onFinish} layout="vertical">
            <Row gutter={[20, 20]}>
                <Col span={24}>
                    <Form.Item
                        label="Tên"
...
const JobByEmail = () => {
    ...
    const onFinish = async (values: any) => {
        ...
        try {
            ...
            const res = isExistingSubscriber ? await callUpdateSubscriber(payload) : await callCreateSubscriber(payload)

            if (res.data) {
                message.success(
                    isExistingSubscriber ? 'Cập nhật thông tin thành công' : 'Đăng ký nhận thông báo thành công',
                )
            }
        } catch (error: any) {
            notification.error({
    ...
    return (
        <Form onFinish={onFinish} form={form}>
            <Row gutter={[20, 20]}>
                ...
                <Col span={24}>
                    <Button loading={isSubmitting} disabled={isLoading} onClick={() => form.submit()}>
